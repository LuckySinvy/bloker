...
class ReportDialogManager(private val context: Context) {
    ...
    private fun setupDialogView(dialogView: View, number: String) {
        ...
        val checkboxTellows = dialogView.findViewById<android.widget.CheckBox>(R.id.checkboxTellows)
        ...
        checkboxTellows.text = buildProviderText(context, context.getString(R.string.reported_to_tellows), getCountryDisplayName())
    }
            ...
            if (checkboxTellows.isChecked) {
                getTellowsApiCountry(context)?.let { country ->
                    if (ApiUtils.reportToTellows(number, message, isSpam, country)) {
                        reportedTo.add(context.getString(R.string.reported_to_tellows))
                    }
                }
            }
