...
object ApiUtils {
    ...
    fun reportToSelfHostedBackend(
        baseUrl: String,
        phone: String,
        comment: String,
        isSpam: Boolean
    ): Boolean {
        val url = baseUrl.trim().removeSuffix("/") + "/api/v1/spam-reports"
        val requestBodyJson = JSONObject().apply {
            put("phone_number", phone)
            put("label", if (isSpam) "telemarketing" else "not_spam")
            put("notes", comment)
        }.toString()
    ...
    fun checkSelfHostedSpamApi(baseUrl: String, phone: String): Boolean {
        val separator = if (baseUrl.contains("?")) "&" else "?"
        val url = baseUrl.trim().removeSuffix("/") +
            "/api/v1/spam-reports/lookup" +
            separator +
            "phone_number=" + java.net.URLEncoder.encode(phone, "UTF-8")
        ...
        return try {
            fastClient.newCall(request).execute().use { response ->
                ...
                if (!json.optBoolean("found", false)) {
                    ...
                    json.optBoolean("should_block", false)
                }
            }
        } catch (e: Exception) {
            false
